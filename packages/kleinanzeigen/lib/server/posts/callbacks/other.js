 ...
 */
function PostNewUpdateUserKarma(item, user, collection, operation, context) {
  const getVotePower = (user) => {
    return 1;
  };
  const votePower = getVotePower(user);
  // const karmaAmount = (operation === 'upvote' || operation === 'cancelDownvote') ? votePower : -votePower;
  Users.update({ _id: item.userId }, { $inc: { 'karma': votePower } });
}

addCallback('posts.new.async', PostNewUpdateUserKarma);
