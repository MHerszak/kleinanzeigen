...
class Vote extends PureComponent {
  ...
  vote(e) {
    ...
    if(!user){
      ...
      this.props.vote({ document, voteType: 'upvote', collection, currentUser: user });
    } 
  }

  hasVoted() {
    const document = this.props.document;
    return hasVotedClient({ document, voteType: 'upvote' })
  }
  ...
  render() {
    return (
      <a href="#" className={this.getActionClass()} onClick={this.vote}>
        <Components.Icon name="heart-o" />
        <span className="vote-count">{this.props.document.baseScore || 0}</span>
      </a>
    );
  }
